...
const AdvantageBox = ({ icon, title, description, bgColor, textColor, paraColor }) => {
    return (
        <div className={`group bg-primary-whiteOpacity sm:bg-primary-white  md:bg-primary-white lg:bg-primary-white xl:bg-primary-white 2xl:bg-primary-white xs:w-full xs:h-[55vw] w-full h-[40vw] md:w-full md:h-[250px] lg:w-full lg:h-[250px] xl:w-full xl:h-[295px] 2xl:w-[340px] 2xl:h-[315px] flex flex-col justify-start items-start xs:p-[2vw_2vw] md:p-[2rem_1.5rem] p-[3rem_1.5rem] rounded-[12px] ${bgColor} transition-all duration-300 ease-in-out hover:bg-primary-blue border-solid   md:border-none lg:border-none xl:border-none 2xl:border-none xs:border-primary-purpleMoreLight xs:border`}>
            ...
            <h3 className={`xs:mt-[1vw] sm:mt[0.7rem] md:mt-[1rem] xl:mt-[1rem] 2xl:mt-[1rem] font-interBold xs:text-[14px] sm:text-[3.8vw] md:text-[20px] text-[24px] ${textColor} mb-[0] group-hover:text-primary-white`}>{title}</h3>
            <p className={`${paraColor} font-interRegular xs:mt-[1vw] sm:mt-[0.7rem]  leading-[14px] sm:leading-[16px] md:leading-[16px] xl:leading-[18px] 2xl:leading-[18px] md:mt-[1rem] mt-[1.5rem] xs:text-[3vw] sm:text-[14px] md:text-[16px] text-18px group-hover:text-primary-whiteSecond`}>{description}</p>
        </div>
    );
};
