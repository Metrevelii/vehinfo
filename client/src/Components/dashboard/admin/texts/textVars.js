...
const TextInput = ({ id, name, formik }) => {
  return (
    <div className="flex items-start justify-center flex-col mt-[20px] w-full">
       <label className="font-interMedium text-primary-blue text-[14px]" htmlFor={id}>Edit {name}</label>
      <input
        type="text"
        id={id}
        name={name}
...
const TextVars = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      dispatch(updateTranslationVars(
          {
  ...
  const AZTextFields = ({ formik }) => {
    const azFields = [
      'text1az', 'text2az', 'text3az', 'text4az', 'text5az', 'text6az',
      'text7az', 'text8az', 'text9az', 'text10az', 'text11az', 'text12az',
      'text13az', 'text14az', 'text15az', 'text16az', 'text17az'
    ];
  ...
  const RUTextFields = ({ formik }) => {
    const ruFields = [
      'text1ru', 'text2ru', 'text3ru', 'text4ru', 'text5ru', 'text6ru',
      'text7ru', 'text8ru', 'text9ru', 'text10ru', 'text11ru', 'text12ru',
      'text13ru', 'text14ru', 'text15ru', 'text16ru', 'text17ru'
    ];
  ...
  return (
    <form className="mt-3" onSubmit={formik.handleSubmit}>
      ...
      {isAZ ? (
        <AZTextFields formik={formik} />
      ) : (
        <RUTextFields formik={formik} />
      )}
      </div>
      <div className="mt-[20px] flex justify-between items-center w-full lg:w-[50%] xl:w-[50%] 2xl:w-[50%]">
      <ButtonComponent className="w-[48%]  md:w-[48%] lg:w-[165px] xl:w-[220px] 2xl:w-[220px] h-[48px] rounded-[12px] font-interMedium text-[16px]" title={"Edit texts"}/>
        <button className="w-[48%] md:w-[48%] lg:w-[165px] xl:w-[220px] 2xl:w-[220px] h-[48px] rounded-[12px] flex justify-center items-center border border-solid border-[#7c8fc7] font-interRegular text-primary-blue text-[16px]" type="button" onClick={toggleLanguage}>
